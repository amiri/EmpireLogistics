-- Revert migrate_labor_types_from_enum
...
BEGIN;
    ...
    UPDATE tmp_labor_organization te SET organization_type = ot.name FROM labor_organization_type ot WHERE te.organization_type::integer = ot.id;

    DROP TABLE labor_organization_type cascade;

    CREATE TYPE labor_organization_type AS ENUM ('federation', 'union', 'hybrid','reform','local','unaffiliated','other');

    ALTER TABLE tmp_labor_organization ALTER COLUMN organization_type SET DATA TYPE labor_organization_type USING (organization_type::labor_organization_type);

    TRUNCATE labor_organization CASCADE;

    ALTER TABLE labor_organization DROP COLUMN organization_type;
    ALTER TABLE labor_organization ADD COLUMN organization_type labor_organization_type;
