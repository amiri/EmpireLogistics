package EmpireLogistics::Form::Field::LaborOrganizationTotalLiability;
...
sub _build_item {
    ...
    my $liability = $self->form->item->labor_organization_total_liabilities->find(
        {
...
sub render_rm_element {
    ...
    my $rel  = $self->parent->form->total_liability_relation;
    return
...
sub validate {
    ...
    $self->field('year')->add_error("Total liability year required")
        if $year_required;
}
