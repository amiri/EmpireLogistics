...
has 'item' =>
    (is => 'rw', clearer => 'clear_item', lazy => 1, builder => '_build_item');

sub _build_item {
    my $self = shift;
    my $address = $self->form->item->addresses->find({ id => $self->field('id')->fif });
    return $address;
}
...
sub render_rm_element {
    ...
    return
        qq{<div><span class="btn rm_element" data-rep-elem-id="$id">Remove</span></div>};
}
