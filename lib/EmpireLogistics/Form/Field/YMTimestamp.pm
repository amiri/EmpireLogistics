package EmpireLogistics::Form::Field::YMTimestamp;
...
has 'format' => ( is => 'rw', isa => 'Str', default => "%Y-%m" );
...
sub validate {
    ...
    warn "Datetime is $dt";
    $self->_set_value($dt);
    my $val_strp = DateTime::Format::Strptime->new( pattern => "%Y-%m" );
    if ( $self->date_start ) {
