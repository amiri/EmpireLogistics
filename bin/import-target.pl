...
my $file = "$dir/target-distribution-centers.json";

my $io  = io($file)->all;
my $dcs = JSON::XS->new->utf8->decode($io);

my @warehouse_types = ("Target Distribution Center");
my @warehouses;
...
for my $dc ( @$dcs ) {
    ...
    try {
        ...
        say "Couldn't reverse geocode ", $latlng, ": $_";
    };
    ...
    my $geom = "$lon $lat";
    my $warehouse = [
        "Target Distribution Center", $street_address, $city,        $state,  $postal_code,
        $country,        $owner,       $geom,
    ];
    push @warehouses, $warehouse;
    say "    Processed Target warehouse ", $street_address;
